...
Uses adjacency rules and spatial constraints to prevent impossible combinations
"""
...
class SpatialConstrainedConstellationMatcher:
    """Constellation matcher with spatial constraints and adjacency rules."""
    ...
    def _create_adjacency_rules(self) -> Dict:
        """Create adjacency rules based on actual constellation positions."""
        return {
            ...
            "Centaurus": ["Crux", "Carina", "Lupus", "Lupus"],  # Centaurus adjacent to Crux, Carina
            "Orion": ["Taurus", "Gemini", "Canis Major", "Canis Minor"],  # Orion adjacent to Taurus, Gemini
            "Ursa Major": ["Cassiopeia", "Draco", "Leo Minor", "Canes Venatici"],  # Big Dipper adjacent to Cassiopeia
            "Cassiopeia": ["Ursa Major", "Perseus", "Andromeda", "Cepheus"],  # Cassiopeia adjacent to Ursa Major
            "Leo": ["Virgo", "Cancer", "Ursa Major", "Coma Berenices"],  # Leo adjacent to Virgo
            "Virgo": ["Leo", "Libra", "Coma Berenices", "Booetes"]  # Virgo adjacent to Leo
        }
    ...
    def find_constellation_patterns(self) -> List[Dict]:
        """Find constellation patterns with spatial constraints."""
        logger.info("🔍 Searching for constellation patterns with spatial constraints...")
        ...
        constrained_matches = self._apply_spatial_constraints(all_matches)
        
        # Sort by confidence
        constrained_matches.sort(key=lambda m: m["confidence"], reverse=True)
        
        logger.info(f"   Found {len(constrained_matches)} spatially-constrained constellation matches")
        return constrained_matches
    
    def _apply_spatial_constraints(self, matches: List[Dict]) -> List[Dict]:
        """Apply spatial constraints to filter out impossible combinations."""
        ...
        # Rule 2: Check adjacency constraints
        valid_combinations = self._check_adjacency_constraints(matches)
        
        # Rule 3: Limit total constellations based on FOV
        ...
        for match in matches[1:]:
            ...
            for valid_match in valid_matches:
                ...
                if const_name in self.adjacency_rules.get(valid_const, []):
                    ...
                    if distance < 200:  # Too close together
                        logger.info(f"   ❌ {const_name} too close to {valid_const} (distance: {distance:.1f})")
                        is_valid = False
                        break
            ...
            if is_valid:
                ...
                logger.info(f"   ❌ Rejected {const_name} (adjacency constraint)")
    ...
    def _get_max_constellations_for_fov(self) -> int:
        ...
        if "narrow_field" in fov_estimate:
            ...
            return 4  # Medium field - 3-4 constellations
        else:
            return 6  # Wide field - up to 6 constellations
    ...
    def create_annotated_image(self, matches: List[Dict], output_path: str = "Output/spatial_constrained_constellations.jpg"):
        """Create annotated image with spatially-constrained constellation patterns."""
        logger.info("🎨 Creating spatially-constrained annotated image...")
    ...
    def _add_info_overlay(self, image: np.ndarray, matches: List[Dict]) -> np.ndarray:
        ...
        # Add title
        title = "Spatially-Constrained Constellation Matching"
        ...
        info_lines = [
            ...
            f"Spatially-valid matches: {len(matches)}",
            f"FOV estimate: {self.fov_info.get('fov_estimate', 'unknown')}",
            f"Method: Pattern matching with spatial constraints"
        ]
...
def main():
    """Test spatially-constrained constellation matching."""
    print("🌍 Spatially-Constrained Constellation Matcher")
    print("=" * 60)
    print("Using adjacency rules and spatial constraints")
    print("=" * 60)
    
    start_time = time.time()
    
    # Create matcher
    matcher = SpatialConstrainedConstellationMatcher()
    ...
    try:
        ...
        print(f"\n✅ Spatially-constrained matching complete!")
        print(f"   Found {len(matches)} spatially-valid constellation matches")
        ...
        if matches:
            ...
            print("   No spatially-valid constellation patterns found")
        ...
        print(f"   Check Output/spatial_constrained_constellations.jpg for results")
